...
{
    // std::cout << "Thread " << std::this_thread::get_id() << " attempting to lock the mutex." << std::endl<<std::flush;
    std::unique_lock<std::mutex> buffer_unique_lock(*(this->_commit_buffer_lock));
    // std::cout << "Thread " << std::this_thread::get_id() << " LOCKED the mutex." << std::endl<<std::flush;
    int ret = 0;
    ...
    this->_commit_buffer_signal->notify_all();
    // std::cout<<"Thread "<< std::this_thread::get_id() << "UNLOCKING the commit buffer"<<std::endl<<std::flush;
    buffer_unique_lock.unlock();
    return ret;
}
...
void LogBuffer::flush_prepare_logs(void) {
    ...
    this->_prepare_buffer_signal->wait_for(buffer_unique_lock, std::chrono::milliseconds(EMPTY_LOG_BUFFER_TIMEDELTA), [&] {
...
void LogBuffer::flush_commit_logs() {
    // std::cout << "FLUSH commit Thread " << std::this_thread::get_id() << " attempting to lock the mutex." << std::endl<<std::flush;
    // std::unique_lock<std::mutex> buffer_unique_lock(*(this->_commit_buffer_lock));
    // while (true) {
        std::unique_lock<std::mutex> buffer_unique_lock(*(this->_commit_buffer_lock), std::defer_lock);
        // std::cout << "FLUSH commit Thread " << std::this_thread::get_id() << " LOCKED the mutex." << std::endl<<std::flush;
        // Wait until there's something to flush or it's time to wake up
        this->_commit_buffer_signal->wait_for(buffer_unique_lock, std::chrono::milliseconds(EMPTY_LOG_BUFFER_TIMEDELTA), [&] {
        ...
        if (!this->_commit_buffer.empty()) {
            ...
            // this->_commit_buffer_signal->notify_all();
            // this->_commit_buffer_signal->notify_one();
        }
        sleep(EMPTY_LOG_BUFFER_TIMEDELTA/1000);
    // }
    
    // std::cout << "FLUSH commit Thread " << std::this_thread::get_id() << " UNLOCKED the mutex." << std::endl<<std::flush;
}
