...
int LogBuffer::add_log(uint64_t node_id, uint64_t txn_id, int status, std::string &data) {
    ...
    string status_data = to_string(status) + ':' + data;
    ...
    if (this->_buffer.size() == DEFAULT_BUFFER_SIZE) {
        ...
        #endif
        ret = spill_buffered_logs_to_storage(*LOGGER, true);
    }
...
int spill_buffered_logs_to_storage(LogBuffer& buf, bool force_empty) {
